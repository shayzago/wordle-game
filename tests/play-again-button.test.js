...
const { mockToastify, unMockToastify } = require("./mocks/toastify");

const { KEY_ENTER, GREEN_COLOR_RGB } = require("./auxi/consts");
...
const getLetterTextContent = (row, index) => {
  return global.document.querySelector(`.row-${row} .letter-${index}`)
    .textContent;
};

const getBoxBackgroundColor = (row, index) => {
  return global.document.querySelector(`.row-${row} .letter-${index}`).style
    .backgroundColor;
};

const getPlayAgainButtonVisibility = () => {
  return global.document.querySelector(`.play-again .btn-play-again`).style
    .display;
};
